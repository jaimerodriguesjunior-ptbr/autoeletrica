  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const formattedData = data?.map((c: any) => ({
        ...
        nome: c.nome,
        whatsapp: c.whatsapp,
        veiculos_count: c.vehicles ? c.vehicles[0].count : 0,
    ...
    <div className="space-y-8 pb-32">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">
            Oficina: <span className="font-bold text-[#1A1A1A]">{profile?.organization_id ? "Conectada" : "Carregando..."}</span>
          </p>
        </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full bg-white pl-14 pr-4 py-5 rounded-[24px] shadow-sm border border-stone-100 text-lg outline-none focus:ring-2 focus:ring-[#FACC15]/50 transition text-[#1A1A1A] placeholder:text-stone-300"
        />
      </div>

      {/* Tabela de Clientes */}
      <div className="bg-white rounded-[32px] shadow-sm border border-stone-100 overflow-hidden min-h-[300px]">
        <div className="p-6 border-b border-stone-50 flex justify-between items-center">
          <h3 className="font-bold text-[#1A1A1A]">Todos os Clientes</h3>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <table className="w-full text-left">
              <thead className="bg-[#F8F7F2] text-stone-500 text-xs uppercase">
                <tr>
                  <th className="px-6 py-4 font-bold rounded-tl-2xl">Nome / Contato</th>
                  <th className="px-6 py-4 font-bold">Veículos</th>
                  <th className="px-6 py-4 font-bold hidden md:table-cell">Cadastro</th>
                  <th className="px-6 py-4 font-bold text-right rounded-tr-2xl">Ação</th>
                </tr>
              </thead>
              <tbody className="text-sm">
                {filteredClients.map((c) => (
                  <tr key={c.id} className="border-b border-stone-50 hover:bg-[#F9F8F4] transition group">
                    <td className="px-6 py-4">
                      <p className="font-bold text-[#1A1A1A] text-lg md:text-base">{c.nome}</p>
                      <div className="flex items-center gap-2 mt-1 text-stone-400 text-xs font-medium">
