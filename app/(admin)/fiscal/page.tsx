        ...
        <div className="space-y-6 pb-32">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-stone-500">Gerencie suas notas fiscais emitidas e importadas.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-[24px] border border-stone-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-2xl font-bold text-[#1A1A1A]">{authorizedOutputs}</p>
                        <p className="text-xs text-stone-500 font-bold uppercase">Emitidas Autorizadas</p>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-[24px] border border-stone-100 shadow-sm flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-red-50 flex items-center justify-center text-red-500"><AlertCircle size={24} /></div>
                    <div>
                        <p className="text-2xl font-bold text-[#1A1A1A]">{erroredOutputs}</p>
                        <p className="text-xs text-stone-500 font-bold uppercase">Emitidas Com Erro</p>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-[24px] border border-stone-100 shadow-sm flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center text-blue-500"><Clock size={24} /></div>
                    <div>
                        <p className="text-2xl font-bold text-[#1A1A1A]">{pendingOutputs}</p>
                        <p className="text-xs text-stone-500 font-bold uppercase">Emitidas Pendentes</p>
                    </div>
                </div>
            </div>
                                ...
                                filteredInvoices.map((inv) => (
                                    <tr key={inv.id} className="border-b border-stone-50 hover:bg-[#F9F8F4] transition">
                                        ...
                                        <td className="px-6 py-4 text-stone-600">
                                            {new Date(displayDate(inv)).toLocaleDateString()}
                                        </td>
                                        <td className="px-6 py-4">
                                            {getStatusBadge(inv.status)}
                                            {inv.error_message && (
                                                <p className="text-[10px] text-red-500 mt-1 max-w-[200px] truncate" title={inv.error_message}>
                                                    {inv.error_message}
                                                </p>
                                            )}
                                            {isEntryInvoice(inv) && (
