  ...
  const fetchProduct = async () => {
    try {
      ...
      if (data) {
        ...
        setCustoContabil(data.custo_contabil.toString());
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-32">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-[#1A1A1A] text-white rounded-[32px] p-6 shadow-xl relative overflow-hidden">
            ...
            <h3 className="font-bold flex items-center gap-2 mb-6"><DollarSign size={18} className="text-[#FACC15]" /> Precificação</h3>

            <div className="space-y-4 relative z-10">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-400 ml-2 flex items-center gap-1">CUSTO REPOSIÇÃO</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-stone-500">R$</span>
                  <input type="number" value={custoReposicao} onChange={(e) => setCustoReposicao(e.target.value)} className="w-full bg-white/10 border border-white/10 rounded-2xl p-4 pl-10 font-bold text-white outline-none focus:border-[#FACC15] transition" />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input type="number" value={margem} onChange={(e) => setMargem(e.target.value)} className="w-full bg-white/10 border border-white/10 rounded-2xl p-4 font-bold text-white outline-none focus:border-[#FACC15] transition" />
              ...
              <div className="bg-white/10 rounded-2xl p-4 mt-4 border border-dashed border-white/20" onClick={() => setPrecoVenda(calcularPrecoSugerido())} role="button">
                <div className="flex items-center gap-2 text-[#FACC15] mb-1"><Calculator size={14} /><span className="text-xs font-bold uppercase">Sugerido (Clique)</span></div>
              ...
              <div className="space-y-1 pt-2">
                ...
                <div className="relative">
                  ...
                  <input type="number" value={precoVenda} onChange={(e) => setPrecoVenda(e.target.value)} className="w-full bg-[#FACC15] text-[#1A1A1A] rounded-2xl p-4 pl-10 font-bold outline-none shadow-lg transition" />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-24 md:bottom-6 right-6 left-6 md:left-auto md:w-96 z-40">
        <button 
          ...
          disabled={saving}
          className="w-full bg-[#1A1A1A] text-[#FACC15] font-bold py-4 rounded-full shadow-lg flex justify-center items-center gap-2 hover:scale-105 transition active:scale-95 disabled:opacity-70"
        >
