...
export default function EditarProduto() {
  ...
  const [dataUltimaCompra, setDataUltimaCompra] = useState(""); // Novo
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (data) {
        ...
        setDataUltimaCompra(data.data_ultima_compra ? new Date(data.data_ultima_compra).toISOString().split('T')[0] : "");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error.message?.includes('foreign key') || error.code === '23503') {
        alert("Este produto não pode ser excluído porque está sendo usado em um ou mais atendimentos (OS). Remova-o dos atendimentos antes de excluir.");
      } else {
        ...
        <div className="space-y-6">
          <div className="bg-[#1A1A1A] text-white rounded-[32px] p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              {/* --- 0. DATA DA COMPRA (NOVO) --- */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-400 ml-2">DATA DA ÚLTIMA COMPRA</label>
                <input
                  type="date"
                  value={dataUltimaCompra}
                  onChange={e => setDataUltimaCompra(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 font-bold text-white outline-none focus:border-[#FACC15] transition focus:bg-white/10"
                />
              </div>
