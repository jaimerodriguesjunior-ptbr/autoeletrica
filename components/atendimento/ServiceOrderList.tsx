...
export default function ServiceOrderList() {
    ...
    const fetchOrders = async () => {
        ...
        `)
                ...
                .not('vehicle_id', 'is', null) // FILTER FOR OS (Assuming OS has vehicle)
    ...
    const filteredOrders = orders.filter(os => {
        ...
        if (filtroAtivo === "em andamento") return ["aprovado", "em_servico", "aguardando_peca"].includes(os.status);
        if (filtroAtivo === "finalizados") return ["pronto", "entregue", "cancelado"].includes(os.status);
                ...
                <div className="flex bg-stone-200 p-1.5 rounded-2xl overflow-x-auto scrollbar-hide border-2 border-stone-300 shadow-inner gap-1">
                    {["todos", "em andamento", "orçamentos", "finalizados"].map((tab) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold capitalize whitespace-nowrap transition border-2 ${filtroAtivo === tab ? "bg-white text-[#1A1A1A] shadow-md border-stone-300" : "text-stone-500 hover:text-[#1A1A1A] border-transparent"}`}
                        >
                            {tab}
                        </button>
                    ))}
